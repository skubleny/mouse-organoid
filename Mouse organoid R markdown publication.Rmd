...
mouseorganoidR <- read_excel("~/Documents/PhD/Results: spreadsheets/Mouse organoid data/mouseorganoidR.xlsx",
...
#### Overall pooled effects of treatment and treatment time on viability and growth rate

```{r, warning=FALSE}
viabilityplot = ggplot(mouseorganoidR, aes(x=treatment, y=viability)) + geom_boxplot() +ylim(0,100) + facet_wrap(~treatment_time)
viabilityplot
growthplot = ggplot(mouseorganoidR, aes(x=treatment, y=growth_rate)) + geom_boxplot() +ylim(-1,1) + facet_wrap(~treatment_time)
growthplot
```

## Let's investigate the trends of viability and growth rate according to treatment group and treatment times.

#Univariable viability and grwoth rate by time and by media
```{r}
...
univar_media_viability = ggplot(mouseorganoidR, aes(x = treatment, y = viability, fill=treatment)) + 
  ...
  ylab("Viability (%)") +
  ylim(0,100) +
  stat_compare_means(method="anova",size = 5, label.y = 10, label.x=2, hjust=0.5) +
  theme_classic() +
...
univar_time_viability = ggplot(mouseorganoidR, aes(x = treatment_time, y = viability, fill=treatment_time)) + 
  ...
  theme(axis.title.y = element_text(colour="black", size=14)) +
  theme(legend.position = "none") 
  ggsave("univar_time_viability.svg", univar_time_viability, width=3.7, height=4)
...
univar_media_GR = ggplot(mouseorganoidR, aes(x = treatment, y = growth_rate, fill=treatment)) + 
  ...
  ylim(-0.55,1) +
  stat_compare_means(method="anova",size = 5, label.y = 0.9, label.x=0.7, hjust=0) +
  theme_classic() +
  theme(axis.text.x = element_text(colour="black", size = 14))  +
  theme(axis.text.y = element_text(colour="black",size = 14)) + 
  theme(plot.title = element_text(colour="black", size=14,hjust = 0, vjust=0)) +
...
univar_time_GR = ggplot(mouseorganoidR, aes(x = treatment_time, y = growth_rate, fill=treatment_time)) + 
  ...
  theme(axis.title.y = element_text(colour="black", size=14)) +
  theme(legend.position = "none") 
  ggsave("univar_time_GR.svg", univar_time_GR, width=3.7, height=4)
...
```{r, warning=FALSE}
# Set up new data frame and omit missing data
mouse = mouseorganoidR[,c("treatment","treatment_time","passage","viability")]
mouse= na.omit(mouse)
mouse$passage = as.factor(mouse$passage)
mouse_gr = mouseorganoidR[,c("treatment","treatment_time","passage","growth_rate")]
mouse_gr = na.omit(mouse_gr)
mouse_gr$passage = as.factor(mouse_gr$passage)
```

## Overall trend of VIABILITY and GROWTH RATE by passage

## Viability adjusted by days in culture, treatment and treatment time

A note about methods: Here I will perform an adjusted response plot. I will adjust the variables of viability and growth rate with variables of interest. This is similar to adjusting for batch effects or other known variables of interest. We only have two independent variables thus I fit each individual regression and used the predict() function to find my fitted values From those models the fitted values can be found as well as residuals etc using mod$fitted.values.
...
#Assess growth ratespline vs gam
...
#Gam wins in the full model this improves over the basic model from adj r squared of 0.34 to 0.4058 to 0.444
...
#Reduced model not significantly improved over full model using liklihood ratio test. 
...
flextable::save_as_docx("tb_viability_gam" = tb_viability_gam_flex,path = "/Users/danielskubleny/Documents/R projects/Mouse Organoid/tb_viability_gam.docx")
...
agg.treatment_adjviability = aggregate(formula = adjviability ~ treatment + passage, FUN = mean, data=mouse_adjviability)
agg.treatment_adjviabilityplot = ggplot(agg.treatment_adjviability, aes(x=passage, y=adjviability, group=treatment, colour=treatment)) + geom_line() + geom_point() + ylim(0,100) + theme(legend.position="bottom")
agg.time_adjviability = aggregate(formula = adjviability ~ treatment_time + passage, FUN = mean, data=mouse_adjviability)
agg.time_adjviabilityplot = ggplot(agg.time_adjviability, aes(x=passage, y=adjviability, group=treatment_time, colour= treatment_time)) + geom_line() + geom_point() + ylim(0,100) + theme(legend.position="bottom")
...
## Growth rate adjusted by days in culture, treatment and treatment time

```{r, warning=FALSE}
# Growth rate is significantly affected by the number of days in culture. Thus to demonstrate true effect of treatment groups I adjusted the viability values using a generalized linear model. 
glm2 = glm(mouseorganoidR$growth_rate ~ mouseorganoidR$days_culture + mouseorganoidR$treatment + mouseorganoidR$treatment_time + mouseorganoidR$passage)
glm2 = glm(mouseorganoidR$growth_rate ~ mouseorganoidR$days_culture +mouseorganoidR$cohort + mouseorganoidR$treatment + mouseorganoidR$treatment_time + mouseorganoidR$passage)
predictglm2 = predict(glm2)
mouse_adjgrowth = mouseorganoidR[,c("treatment","treatment_time","passage","growth_rate")]
mouse_adjgrowth = na.omit(mouse_adjgrowth)
mouse_adjgrowth$adjgrowth = predictglm2
# Aggregate data and make plots for media and time
agg.treatment_adjgrowth = aggregate(formula = adjgrowth ~ treatment + passage, FUN = mean, data=mouse_adjgrowth)
agg.treatment_adjgrowthplot = ggplot(agg.treatment_adjgrowth, aes(x=passage, y=adjgrowth, group=treatment, colour=treatment)) + geom_line() + geom_point() + ylim(0,0.5) + theme(legend.position="bottom")
agg.time_adjgrowth = aggregate(formula = adjgrowth ~ treatment_time + passage, FUN = mean, data=mouse_adjgrowth)
agg.time_adjgrowthplot = ggplot(agg.time_adjgrowth, aes(x=passage, y=adjgrowth, group=treatment_time, colour=treatment_time)) + geom_line() + geom_point() + ylim(0,0.5) + theme(legend.position="bottom")
```

#Complex time models for growth rate 
The growth rate model glm2 was quite poor with an adjusted R2 of 0.07! Thus I investigated why this might be. Obviously it is possible that with an assessment of growth over time there could be different effects of days in culture and passage on growth rate. So I assessed natural cubic splines and gams. Ultimately gams were the superior model with an adjusted R2 of still only 0.18... thus, our understanding of growth rate is incomplete. 
```{r}
#Viability
require(splines, quietly=TRUE)
...
flextable::save_as_docx("tb_growthrate_gam" = tb_growthrate_gam_flex,path = "/Users/danielskubleny/Documents/R projects/Mouse Organoid/tb_growthrate_gam.docx")
...
## Let's investiage apoptosis assay in relation to treatment and treatment time. We can later compare these to downstream effects such as growth rate, viability and stem cell populations. 
...
lm2 = lm(diss_viability ~ temp_media + cohort + age +treatment + treatment_time, data=multivar_dissviability )
summary(lm2)
lm2_effect = ggpredict(lm2, terms = c("temp_media", "treatment"))
plot(lm2_effect) + coord_cartesian(ylim=c(0,100), xlim=c(0,15))
plot(lm2_effect, add.data=TRUE,ci=FALSE)

lm2_effect_temp = ggpredict(lm2, terms = c("temp_media", "treatment"))


media_marginaleffect = ggplot(lm2_effect_temp, aes(x=x, y=predicted, colour=group)) + 
  geom_line(size=1.25) +
  geom_point(inherit.aes = FALSE, data=multivar_dissviability, aes(x=temp_media, y=diss_viability, fill = treatment_time), size=3.5, shape=21) +
  ylab("Dissociation Viability (%)") +
  xlab("Media Temperature") +
  ggtitle("Marginal effect of mock shipment") +
  ylim(0,100) +
  scale_colour_manual(name = "Media",values=c("#E41A1C", "#377EB8" ,"#4DAF4A")) +
  scale_fill_manual(name="Dissociation Time", labels=c("24 Hours", "48 Hours"), values = c("#A6BDDB", "#1C9099")) + 
  theme_classic() +
    theme(axis.text.x = element_text(colour="black", size = 12)) +
    theme(axis.text.y = element_text(colour="black",size = 12)) + 
    theme(plot.title = element_text(colour="black", size=12,hjust = 0, vjust=0)) +
...
ggsave("media_marginaleffect.svg", media_marginaleffect, width=5, height=3)

```
#Figure 2I
```{r}
lm2_effect_age = ggpredict(lm2, terms = c("age", "treatment"))

age_marginaleffect = ggplot(lm2_effect_age, aes(x=x, y=predicted, colour=group)) + 
  geom_line(size=1) +
  geom_point(inherit.aes = FALSE, data=multivar_dissviability, aes(x=age, y=diss_viability, fill = treatment_time), size=3.5, shape=21) +
  ylab("Dissociation Viability (%)") +
  xlab("Age of mouse") +
  ggtitle("Marginal effect of mock shipment") +
  ylim(0,100) +
  ...
  theme_classic() +
    ...
    theme(legend.title = element_text(color = "black", size = 12),
        legend.text = element_text(color = "black", size = 12))
ggsave("age_marginaleffect.svg", age_marginaleffect, width=5, height=3)
```
#Regression table Figure 2J
```{r}
...
flextable::save_as_docx("tb_diss_viability_regress" = tb_diss_viability_regress_flex,path = "/Users/danielskubleny/Documents/R projects/Mouse Organoid/tb_diss_viability_regress.docx")

  
```

#### Summary of dissociation time and the effects of age, ischemic time and temperature of the media.It appears that whatever effect these variable may have is explained away by the specific cohort. Perhaps there is an effect of the technique used to isolate the organoids. For example, one would expect thet ischemic time would result in decreased viability, however perhaps the technique is removing non-viable cells. 

#Correct TROY and LGR5 
```{r}
...
lgr5_media = mouseorganoidR_control %>%
  ...
  geom_boxplot() + 
  geom_hline(yintercept = 0, linetype=2, alpha=0.5) +
  stat_compare_means(size=5.5) +
scale_y_continuous(name="Log2 relative LGR5 expression",limits = c(-7.5, 5), breaks = c(-7.5,-5,-2.5,0.0,2.5,5),labels = c(-7.5,-5,-2.5,0.0,2.5,5)) +  
  scale_x_discrete(name = "Treatment", labels=c("HBSS", "UW", "HTK")) +
    scale_fill_manual(name="Media",labels=c("HBSS", "UW","HTK"), values = c("#E41A1C", "#377EB8" ,"#4DAF4A")) +
theme_classic() +
...
lgr5_passage = mouseorganoidR_control_passage %>%
  ...
  geom_hline(yintercept = 0, linetype=2, alpha=0.5) +
  geom_boxplot(inherit.aes=FALSE, aes(x = passage_name, y = LGR5_center,fill=passage_name)) + 
  ...
  stat_compare_means(size=5, label.y = -7, label.x= 1, hjust = 0) + # Add global p-value
...
troy_passage = mouseorganoidR_control_passage %>%
  ...
  ggplot(aes(x = passage_name, y = TROY_center)) + 
  geom_hline(yintercept = 0, linetype=2, alpha=0.5) +
  geom_boxplot(inherit.aes=FALSE, aes(x = passage_name, y = TROY_center,fill=passage_name)) + 
  scale_x_discrete(name="Passage number",labels=c("Whole Stomach","Passage 0", "Passage 1","Passage 6")) +
  scale_fill_manual(name="Passage number",labels=c("Whole Stomach", "Passage 0", "Passage 1","Passage 6"), values = c("#FC8D59","#FEE090", "#3288BD","#D53E4F")) +
...
flextable::save_as_docx("tb_lgr5_gam" = tb_lgr5_gam_flex,path = "/Users/danielskubleny/Documents/R projects/Mouse Organoid/tb_lgr5_gam.docx")
...
flextable::save_as_docx("tb_troy_gam" = tb_troy_gam_flex,path = "/Users/danielskubleny/Documents/R projects/Mouse Organoid/tb_troy_gam.docx")

#######

cheese = plot(troy_gam_plot, select = 2)
ggplot(data_cheese, aes(x = x, y=y)) + geom_point(colour="red") + geom_errorbar( aes(ymin=y-(1.96*se), ymax=y+(1.96*se)), width=0.4, colour="orange", alpha=0.9, size=1.3)
...
anova(lgr5_lm,lgr5_gam)
anova(troy_lm,troy_gam)
#Simple model is adequate. 

ggpredict(troy_lm, terms = c("cohort")) %>% plot()
ggpredict(troy_lm, terms = c("passage_name", "cohort")) %>% plot()


lm(TROY_center~ passage_name + cohort+ treatment + treatment_time,data = mouseorganoidR_control_passage)
lgr5_lm = lm(LGR5_center~ passage_name + cohort+ treatment + treatment_time,data = mouseorganoidR_control_passage)
```
...
cohort5_dose <- read.csv("~/Documents/R projects/Mouse Organoid/cohort5_dose.csv", row.names=1)
...
#Add in ic50 values from Graphpad analysis
mean_troy <- read.csv("~/Documents/R projects/Mouse Organoid/mean_troy.csv", row.names=1)
...
#Annexin  
```{r}
annexin_long = mouseorganoidR
annexin_long = dplyr:: select(annexin_long, c("cohort","treatment", "treatment_time", "live_annexin", "dead_annexin", "apoptosis_annexin"))
annexin_long = tidyr::gather(annexin_long, condition, measurement, live_annexin:apoptosis_annexin, factor_key=TRUE)
annexin_long = na.omit(annexin_long)

annexin_long_SE = Rmisc::summarySE(annexin_long, (measurevar="measurement"), groupvars=c("condition", "treatment_time"))

annexin_long_SE_tx = Rmisc::summarySE(annexin_long, (measurevar="measurement"), groupvars=c("condition", "treatment"))

```
#Annexin plots for paper
```{r}
annexin_treat = ggplot(annexin_long_SE, aes(x = condition, y =measurement, fill =  treatment_time)) + 
    geom_bar(stat="identity", color="black", position=position_dodge()) + 
    geom_errorbar(aes(ymin = measurement,ymax = measurement+sd),position=position_dodge(.9), width=.2) + 
    stat_compare_means(inherit.aes = FALSE, data=annexin_long, aes(x = condition, y =measurement, fill =  treatment_time), size=3.7, label.y = 0.9) +
    ylab("Proportion") +
    ylim(0, 1) +
    xlab("Cell Status") +
    scale_x_discrete(labels=c("Live", "Dead", "Apoptosis")) +
    scale_fill_manual(name="Dissociation Time",labels=c("Fresh", "24 Hours","48 Hours"), values = c("#ECE2F0", "#A6BDDB", "#1C9099")) +
    theme_classic() +
    theme(axis.text.x = element_text(colour="black", size = 14)) +
    theme(axis.text.y = element_text(colour="black",size = 14)) + 
...
ggsave("annexin_treat.svg", annexin_treat, width=6, height=4)


annexin_media = ggplot(annexin_long_SE_tx, aes(x = condition, y =measurement, fill =  treatment)) + 
    geom_bar(stat="identity", color="black", position=position_dodge()) + 
    geom_errorbar(aes(ymin = measurement,ymax = measurement+sd),position=position_dodge(.9), width=.2) + 
    stat_compare_means(inherit.aes = FALSE, data=annexin_long, aes(x = condition, y =measurement, fill =  treatment), size=3.7, label.y = 0.9) +
    ylab("Proportion") +
    ylim(0, 1) +
    xlab("Cell Status") +
    scale_x_discrete(labels=c("Live", "Dead", "Apoptosis")) +
    ...
    theme(legend.title = element_text(color = "black", size = 14),
          legend.text = element_text(color = "black", size = 14),
          legend.position = "bottom")
ggsave("annexin_media.svg", annexin_media, width=6, height=4)
```
...
#option "#A6BDDB", "#3690C0", "#02818A"
#final option "#D0D1E6", "#3690C0", "#016C59"
#option "#ECE2F0", "#A6BDDB", "#1C9099"
mypal <- pal_npg("nrc")(9)
...
```
#Figure 2F: dissociation viability vs Treatment time 
```{r}
...
#Figure 2E: dissociation viability vs Treatment for 24 and 48 hours. 
```{r}
#Figure 2E
...
summary(mouseorganoidR %>%
  filter(!treatment_time=="Fresh") %>% 
  lm(diss_viability~treatment+cohort, data=.))

haha = mouseorganoidR %>%
  filter(!treatment_time=="Fresh") %>% 
  lm(diss_viability~treatment+cohort+ treatment_time, data=.)
ggpredict(haha, terms= c("treatment")) %>% plot()
```

#Figure 2G: dissociation viability vs Cohort 
```{r}
#Figure 2G
